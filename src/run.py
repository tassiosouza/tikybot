...
while True:
    ...
    if (len(account_list) > 0):
        ...
        hasCredential = "credentials" in current_user
        if(hasCredential):
            current_login = current_user["credentials"]["username"]
            current_password = current_user["credentials"]["password"]
            current_uid = current_user["uid"]

            if(tikybot.login(current_login, current_password, current_uid) == False):
                continue
            ...
            if current_follow_count < max_follow_count:
                ...
                timeout = random.randrange(300 , 500)
            ...
            print('Conta registrada - ' + current_user["username"] + ' - Sem credentials')
    else:
        print('Nenhuma conta registrada')
    time.sleep(3)
    current_account += 1
    if(current_account >= len(account_list)):
        current_account = 0;
